...
<div class="page row">
    <div class="col s12 m8 offset-m2">
        ...
        <form action="{{ action('HelpController@store') }}" method="post"> @csrf
            ...
            <div class="input-field"> {{-- Title field --}}
                ...
                <textarea name="title" id="title" class="materialize-textarea counter" data-length="{{ config('valid.help.title.max') }}" required>{{ old('title') }}</textarea>
            </div>

            <div class="input-field"> {{-- Text field --}}
                <label for="text">@lang('help.help_text')</label>
                <textarea name="text" id="text" class="materialize-textarea counter" data-length="{{ config('valid.help.text.max') }}" required>{{ old('text') }}</textarea>
            </div>
        </form>
    </div>
</div>
