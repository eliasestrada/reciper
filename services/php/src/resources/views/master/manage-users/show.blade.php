    ...
    <div class="center col s12 m6">
        ...
        @if ($user->id != 1)
            @if ($user->isBanned())
                ...
                <div class="pt-2">
                    <form class="row"
                        action="{{ action('Master\ManageUserController@destroy', ['id' => $user->id]) }}"
                        method="post"
                        onsubmit="return confirm('@lang('manage-users.are_you_sure_to_unban')')"
                    >
                        @csrf @method('delete')
    ...
    <div class="col s12 m6">
        <table class="mt-3 responsive">
            <tbody>
                <tr> {{-- Last activity --}}
                    <td>
                        @lang('visitors.last_activity'): 
                        <span class="red-text">{{ $user->updated_at }}</span> 
                        ({{ time_ago($user->online_check) }})
                    </td>
                </tr>
                <tr>{{-- Firt visit --}}
                    <td>
                        @lang('visitors.first_visit'): 
                        <span class="red-text">{{ $user->created_at }}</span> 
...
@if ($user->id != 1 || !$user->isBanned())
    <div id="ban-user-modal" class="modal">
        <div class="modal-content reset">
            {{-- Bun user form --}}
            <form class="row"
                action="{{ action('Master\ManageUserController@update', ['id' => $user->id]) }}"
                method="post"
            >
                ...
                <div class="input-field col s12 m9">
                    <textarea name="message"
                        ...
                        data-length="{{ config('valid.feedback.ban.message.max') }}"
                        maxlength="{{ config('valid.feedback.ban.message.max') }}"
                        required
                ...
                <button class="btn red confirm"
                    type="submit"
                    data-confirm="@lang('manage-users.are_you_sure_to_ban')"
                >
