    ...
    <div class="page">
		...
		<div class="row">
			<div class="col l3">
				...
				<div class="mb-4">
					<ul class="collection">
						<li class="collection-header px-2"><h5><i class="material-icons right">equalizer</i> Levels</h5></li>

						@foreach($log->menu() as $levelKey => $item)
							@if ($item['count'] === 0)
								<a class="collection-item d-flex justify-content-between align-center text-dark">
									<span class="level-name">{!! $item['icon'] !!} {{ $item['name'] }}</span>
									<span class="badge">{{ $item['count'] }}</span>
								</a>
							@else
								<a href="{{ $item['url'] }}" class="collection-item d-flex justify-content-between align-center text-dark {{ $level === $levelKey ? ' main-light' : ''}}">
									<span class="level-name">{!! $item['icon'] !!} {{ $item['name'] }}</span>
