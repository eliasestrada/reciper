					...
					@admin
						@if (!$user->isAuthor() )
							<div class="block-message-content">
								...
								<a href="/member/{{ $user->id }}/add" class="button-add-user" title="Нажмите чтобы добавить этого пользователя к команде" onclick='return confirm("Вы точно хотите добавить этого пользователя как автора рецептов?")'>Добавить к команде</a>

								<a href="/member/{{ $user->id }}/delete" class="button-add-user" title="Нажмите чтобы удалить этого пользователя" onclick='return confirm("Вы точно хотите удалить этого пользователя?")'>Удалить</a>
							</div>
						@endif
					@endadmin
