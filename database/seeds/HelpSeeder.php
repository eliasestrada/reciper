    ...
    public function run()
    {
        Help::insert([
            ...
            [
                ...
                'text_ru' => 'Все рецепты которым вы подарили сердечко остаются отмеченными для вас, для того чтобы иметь быстрый доступ к ним в любое время. Все подаренные сердца привязаны к вашему устройству, поэтому при смене устройства вы не обнаружите их в списке <a href="' . \URL::to('/recipes#my_likes') . '">Любимые</a> рецепты. Для того чтобы рецепты отображались на всех устройствах вам необходимо добавлять рецепты в "Избранное" нажимая на звёздочку <i class="fas fa-star" style="color:darkgoldenrod"></i>. Добавлять рецепты в "Избранное" могут только зарегистрированные пользователи.',
            ],
            [
                'id' => 8,
                'help_category_id' => 3,
                'title_ru' => 'Как отбираются простые рецепты?',
                'text_ru' => '<b>Быстрый ответ:</b> Время приготовления должно быть не больше <b>60-ти</b> минут и количество ингредиентов в сумме с количеством шагов  приготовления не дожно превышать <b>10</b><br><br><b>Подробный ответ:</b> Среди всех рецептов ресипёров мы отбираем самые простые в приготовлении, и добавляем их в категорию "Простые".<br>Для того чтобы рецепт попал в эту категорию, мы делаем не сложный подсчет количества ингредиентов в рецепте, количества шагов которое читателю предстоит сделать для приготовления блюда, время приготовления блюда, и получаем результат.<br>К примеру, у нас рецепт, в котором <span class="red-text">5</span> ингредиентов, процесс приготовления состоит из <span class="red-text">4</span> шагов а время приготовления <span class="red-text">50</span> минут. При сложении ингридиентов и шагов итоговое число <span class="red-text">9</span>.<br>Если итоговое число <b>меньше 10-ти</b> и время приготовления <b>не больше одного часа</b>, то рецепт отбирается в категорию простые рецепты.<br>В нашем примере у нас итоговое число <span class="red-text">9</span>, девять меньше десяти, значит рецепт простой, тем более врямя приговления всего 50 минут. Вот если бы время было больше 60 минут, то рецепт бы никогда не попал в список простых в приготовлении.',
            ],
            // [
            //     'id' => 9,
            //     'help_category_id' => 1,
